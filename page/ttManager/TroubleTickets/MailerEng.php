...
//            $mail->send($to);
            $headers = array('id', 'Zamowienie', 'NrNetWorkS', 'NazwaStacji', 'Opis', 'Uwagi', 'Kategoria', 'Subco', 'ZgloszeniePL','DataUsunieciaPrzezSub', 'DataWyslaniaDoSUB', 'DataZgloszeniaUsunieciaPL', 'DataZamknieciaUsterek', 'OsobaKontaktowa', 'author', 'region', 'Wlasciciel');
            $tt = $this->add('Model_ReportAll', $headers)->addCondition('csc', $_GET['id']);
//            $this->api->stickyGet('sel_tts');
            $rw = $tt->getRows();
            //die(var_dump($rw));
            $this->writeReport($rw, './Reports/'.$rw[0]['csc'].'_usterki.xls', 'TTs');
            ...
            $mail->addPart('Attachment')->set('./Reports/'.$rw[0]['csc'].'_usterki.xls','application/vnd.ms-excel',$rw[0]['csc'].'_usterki.xls');
            $mail->set('subject', $rw[0]['csc'].'_'.$rw[0]['subco'].' nProj Engineer mail');
    ...
    function writeReport($rs, $fn, $report_name) {
        ...
        $filterCols = array('csc', 'id', '1', '2', '3', 'kod', 'opis', 'Uwagi', 'DataUsunieciaPrzezSub','ttowner');
        ...
        for ($i = 0; $i < $max_col; $i++) {
            $worksheet->write(0, $i + 10, 'Uwagi_' . ($i + 1), $format_title_red);
            $worksheet->setMerge(0, $i + 10, 1, $i + 10);
        }
        $workbook->close();
//        $js = array();
